package Tetriminos;

import Settings.SETS;
import Settings.SETS_GAME;

import java.awt.*;

public class Itetrimino extends Tetrimino{

    public Itetrimino() {
        color = new Color(244,33,194);
        rotate_state = 0;
        blocks = new Block[4];
        blocks[0] = new Block(SETS_GAME.MIDDLE,0,color);
        blocks[1] = new Block(SETS_GAME.MIDDLE,SETS_GAME.SIZE,color);
        blocks[2] = new Block(SETS_GAME.MIDDLE,2*SETS_GAME.SIZE,color);
        blocks[3] = new Block(SETS_GAME.MIDDLE,3*SETS_GAME.SIZE,color);
    }

    @Override
    public void rotate() {
        switch (rotate_state){
            case 0:

                if(blocks[0].getX() == SETS_GAME.MIN_X ||
                        blocks[0].getX() > SETS_GAME.MAX_X - 3*SETS_GAME.SIZE)
                    break;



                blocks[0].setY(blocks[1].getY());
                blocks[0].setX(blocks[1].getX()-SETS_GAME.SIZE);

                blocks[2].setY(blocks[1].getY());
                blocks[2].setX(blocks[1].getX()+SETS_GAME.SIZE);

                blocks[3].setY(blocks[1].getY());
                blocks[3].setX(blocks[1].getX()+2*SETS_GAME.SIZE);

                this.rotate_state = 1;
                break;

            case 1:
                if (blocks[0].getY() > SETS_GAME.MAX_Y-SETS_GAME.SIZE*3)//leza poziomo, wystarczy sprawdzic jeden blok
                    break;


                blocks[0].setY(blocks[1].getY()-SETS_GAME.SIZE);
                blocks[0].setX(blocks[1].getX());

                blocks[2].setY(blocks[1].getY()+SETS_GAME.SIZE);
                blocks[2].setX(blocks[1].getX());

                blocks[3].setY(blocks[1].getY()+2*SETS_GAME.SIZE);
                blocks[3].setX(blocks[1].getX());

                this.rotate_state = 0;
                break;
        }
    }


}
